...
// Timer-driven feedback for the 5–15 s wait: Pip reads (bobbing over a page) while the step labels
// advance on timers and the bar is a CSS animation, so the page keeps moving even while nothing has
// come back yet.
const STEPS = [
  { at: 0, label: (n) => `Picking the right ones out of ${n} policies` },
  { at: 3000, label: () => "Reading them clause by clause" },
  { at: 7000, label: () => "Lining up your options and deadlines" },
];
  ...
  return (
    <div className="rounded-2xl border border-line bg-card p-6 shadow-card sm:p-8">
      <div className="flex items-start gap-4 sm:gap-6">
        <Pip pose="reading" bob className="size-20 shrink-0 sm:size-24" />
        <div className="min-w-0 flex-1">
          <div className="flex flex-col gap-y-1 sm:flex-row sm:items-baseline sm:gap-x-6">
            ...
            <p className="shrink-0 text-sm text-muted">
              {slow ? "Still reading — a long answer can take up to a minute" : "usually 5–15 seconds"}
            </p>
          </div>
