...
- `npm run ingest`: 6 docs / 602 clause chunks in `data/` (PPM 160-9 unservable upstream). Committed.
...
- `npm run eval`: hit@1 11/22, hit@3 16/22, hit@6 17/22; THRESHOLD 3.5 refuses 0 answerable and 2/8
  off-corpus questions at gate 1; mock end-to-end 24/30.
...
- The real-key path (`ANTHROPIC_API_KEY` set) has never been exercised. Gate 2 (`NO_ANSWER`) is untested,
  and on this corpus it carries most of the refusal load — "How much is a parking ticket?" scores 3.71 and
  is answered in mock mode.
- The UI in a real browser at 1336–2560 px: chip→card highlight and layout are implemented but unseen.
- Retrieval hit@3 target is 80%; day 1 is 73%. The remaining misses are paraphrase gaps BM25 cannot close
