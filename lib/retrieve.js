...
/**
 * Gate 1 threshold on `normScore` (see search()). Calibrated by `npm run eval` on the 7-doc corpus
 * (2026-09-17): answerable top scores 3.76–26.02, off-corpus questions 1.58–10.13. The ranges
 * overlap, so no value separates them; 3.5 sits just under the lowest answerable score (refuses
 * nothing the policies cover) and only catches the clearly off-topic tail. MIN_COVERAGE below
 * catches more; gate 2 — the LLM answering NO_ANSWER — carries the rest. Re-run the eval whenever
 * the corpus or the index options change.
 */
export const THRESHOLD = 3.5;

/**
 * Gate 1's second condition: the share of the question's content terms that match anything in the
 * corpus. BM25 lets one rare shared word carry the whole score — "How much is a parking ticket?"
 * clears THRESHOLD on "parking" alone — but a question whose other words never appear in the
 * policies is off-corpus. On the eval 0.5 refuses no answerable question (q05 and q11 sit exactly
 * on it, so there is no margin below) and catches the off-topic questions THRESHOLD lets through.
 */
export const MIN_COVERAGE = 0.5;

// Function words plus the campus name. Every student question says "ucsd"/"I"/"my" but the corpus
// almost never does (11 of 638 chunks contain "ucsd"), so those terms had huge idf and pulled the
 ...
 * short questions are comparable and THRESHOLD can be a single number. `coverage` (the same on
 * every hit) is the share of those terms matched by any chunk in the full ranking — prefix matches
 * included, so it mirrors exactly what the index can see.
 */
export function search(question, k = 6) {
  const termCount = tokenize(question).map(processTerm).filter(Boolean).length || 1;
  const hits = index.search(question); // every chunk matching at least one term, best first
  const coverage = new Set(hits.flatMap((r) => r.queryTerms)).size / termCount;
  return hits
    .slice(0, k)
    .map((r) => ({ chunk: byId.get(r.id), score: r.score, normScore: r.score / termCount, coverage }));
}

export function isConfident(results) {
  return results.length > 0 && results[0].normScore >= THRESHOLD && results[0].coverage >= MIN_COVERAGE;
}
