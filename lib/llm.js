...
Answer only from the documents provided. If they do not cover the topic, reply with exactly ${NO_ANSWER} and nothing else. If they cover the topic but say nothing about the specific point asked, answer with Verdict: n/a and a bold line that says what they do not cover — never turn their silence into a rule or a "no".
...
Verdict: yes, no, depends, or n/a (n/a for how-to questions and for points the documents do not address)
...
let client;
// A warm answer takes 5–15 s and a cold area write ~20 s; the SDK's default 10-minute timeout would let
// one hung connection eat the whole request (two eval runs saw calls stall past 90 s during an API
// slowdown). 40 s and one retry keep the worst case inside the route's maxDuration.
const api = () => (client ??= new Anthropic({ timeout: 40_000, maxRetries: 1 }));
...
export async function answer({ question }) {
  ...
  if (!routed.area) {
    // The router said no area covers this (explicit null): a refusal for the router's price, no read.
    return {
      answer: NO_ANSWER, citations: [],
      grounding: {
        area: null, areaName: null, documents: 0, routed: [], retried: false,
        inputTokens: 0, cacheRead: false, model: ROUTER_MODEL, stopReason: 'no_area', cost: round(routed.cost),
      },
    };
  }
...
async function ask(question, areaId) {
  ...
  try {
    res = await api().beta.messages.create({
      model: MODEL,
      // Answers run ~600–1000 tokens; 2000 keeps Deadlines (the last section) from being cut off (one
      // 69-question run still hit 1600 once) and caps the worst-case output cost at 2¢ on Sonnet 5.
      // stop_reason is logged and reported either way.
      max_tokens: 2000,
...
// ~0.2¢). An explicit {"area": null} means no area covers the question and answer() refuses without a
// read; any other reply that is not an area id falls back to the keyword router.
async function route(question) {
  ...
  if (!area) {
    // An explicit null is the router's decision (no group of policies covers this): refuse without a read.
    // Anything else — no JSON, an unknown id — is a router failure, and the keyword router stands in.
    if (/"area"\s*:\s*null/.test(text)) return { area: null, second: null, cost };
