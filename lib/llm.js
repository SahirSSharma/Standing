...
async function ask(question, areaId) {
  ...
  try {
    res = await api().beta.messages.create({
      model: MODEL,
      // Answers run ~500 tokens with the 20-word rule; 1200 caps the worst-case output cost at 1.2¢ on Sonnet 5.
      max_tokens: 1200,
  ...
  const u = res.usage;
  const base = {
    area: areaId, model: res.model, cost: estimateCost(u, MODEL, `answer/${areaId}`),
...
function mockAnswer(question) {
  ...
  return { answer: text, citations, grounding: { area: area.id, areaName: area.name, documents: area.docs.length,
    routed: [area.id], retried: false, inputTokens: 0, cacheRead: false, model: 'mock', cost: 0 } };
}
...
function estimateCost(u, model, label) {
  ...
  console.log(`[standing] ${label} ${model} in=${u.input_tokens} cacheWrite=${u.cache_creation_input_tokens ?? 0} cacheRead=${u.cache_read_input_tokens ?? 0} out=${u.output_tokens} ≈${cents}¢`);
  return cents;
}
