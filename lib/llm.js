...
// Sonnet 5 by default: with the corpus served from cache an answer costs about $0.02 (vs ~$0.05 on
// Opus 5) and returns faster. STANDING_MODEL overrides it, e.g. claude-opus-5 for the eval comparison.
...
Open with one bold line that directly answers the question as asked (e.g. **Generally no — not without your written consent.**), then the explanation.
...
const DOCUMENTS = docs.map((d, i) => ({
  ...
  // 1-hour TTL: traffic is sporadic (a student now, a judge in 40 minutes), and re-writing the 74k-token
  // corpus into the cache costs far more than keeping it warm.
  ...(i === docs.length - 1 && { cache_control: { type: 'ephemeral', ttl: '1h' } }),
}));
 ...
 * each id once, with the cited words from that clause; `grounding` is the token accounting the route reports.
 */
export async function answer({ question }) {
  ...
  try {
    res = await client.beta.messages.create({
      model: MODEL,
      // Answers run ~600 tokens; 1500 caps the worst-case output cost at about 1.5¢ on Sonnet 5.
      max_tokens: 1500,
  ...
  const grounding = {
    ...
    // Rough spend per call in the server log (Sonnet 5 list prices), so the budget is visible.
    cost: estimateCost(u),
    model: res.model,
  };
  ...
  for (const block of res.content) {
    ...
    for (const c of block.citations ?? []) {
      for (const span of resolve(c.document_index, c.start_char_index, c.end_char_index)) {
        const { id } = span.chunk;
        if (ids.has(id)) continue;
        ids.add(id);
        if (!citations.some((x) => x.id === id)) citations.push({ id, quote: quote(c, span) });
      }
    }
...
// Sonnet 5 list prices per token: input $2/M, cache write $2.50/M, cache read $0.20/M, output $10/M.
function estimateCost(u) {
  const usd = (u.input_tokens ?? 0) * 2e-6 + (u.cache_creation_input_tokens ?? 0) * 2.5e-6
    + (u.cache_read_input_tokens ?? 0) * 0.2e-6 + (u.output_tokens ?? 0) * 10e-6;
  const cents = Math.round(usd * 100 * 10) / 10;
  console.log(`[standing] ${MODEL} in=${u.input_tokens} cacheWrite=${u.cache_creation_input_tokens ?? 0} cacheRead=${u.cache_read_input_tokens ?? 0} out=${u.output_tokens} ≈${cents}¢`);
  return cents;
}
