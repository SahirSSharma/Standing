...
npm run ingest        # re-fetch the UCSD PPM documents → data/ (6 of 7, see Corpus)
```
...
- **MiniSearch** — a BM25 index over 602 clause-level chunks, built in memory when the route loads.
...
- No database, no embeddings, no data build step: `data/corpus.json` (379 KB) is committed.
...
2. Gate 1: if the top normalized score is under `THRESHOLD`, refuse without calling the LLM.
...
## Corpus
Six of the seven "Student Matters" documents of the UCSD Policy & Procedure Manual (PPM 160-2, 160-3,
160-6, 160-8, 160-10, 160-11): 602 clause chunks, each carrying its document's effective date and
official URL. PPM 160-9 (Student Organizations) is unservable on UCSD's side — the server reports two
published documents with the same id — so `npm run ingest` skips it with an error line.
...
| Retrieval hit@3 | 16/22 (73%) |
| Retrieval hit@6 (what the LLM is shown) | 17/22 (77%) |
| Gate 1: answerable questions wrongly refused at THRESHOLD 3.5 | 0/22 |
| Gate 1: off-corpus questions refused without an LLM call | 2/8 |
| End-to-end (`LLM_MOCK=1`): answered vs refused correct | 24/30 (80%) |
| End-to-end (`LLM_MOCK=1`): a citation hits an expected clause | 11/22 (50%) |

Reading the numbers: retrieval scores (`normScore`, BM25 ÷ query terms) run 3.68–25.56 for answerable
questions and 1.56–9.97 for off-corpus ones, so no threshold separates them; THRESHOLD 3.5 sits just
below the lowest answerable score and the LLM's `NO_ANSWER` (gate 2) carries the rest. In mock mode
gate 2 never fires, so 24/30 is a floor, and the citation number equals hit@1 because the mock cites
only the top chunk. The five retrieval misses (q07, q09, q15, q20, q21) are paraphrase gaps: the expected
